...
def evaluate():
    ...
    save_dir = ("samples/celeb/%d" % size_y).format(tl.global_flag['mode'])
    ...
    ###========================== DEFINE MODEL ============================###
    t_target_image = tf.placeholder('float32', [1, block_size, block_size, 3], name='t_target_image')
    ...
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_RGB_g.npz', network=net_g)
    ...
    for imid in range(0, len(test_hr_imgs)):
        b_imgs_ = tl.prepro.threading_data(test_hr_imgs[imid:imid+1], fn=crop_sub_imgs_fn, is_random=False)
        b_imgs__ = np.reshape(b_imgs_, [1, block_size, block_size, 3])
        ...
        imgs = np.reshape(b_imgs__, [block_size, block_size, 3])
        out_ = np.reshape(out, [block_size, block_size, 3])
        ...
        if imid % 1 == 0:
            ...
            save_image(out.astype(np.uint8),  save_dir+'/test_gen%d.jpg' % imid)
            b_imgs_hr = np.reshape(b_imgs_, [block_size, block_size, 3])
            b_imgs_hr = tl.prepro.threading_data(b_imgs_hr, fn=inv_norm)
            save_image(b_imgs_hr.astype(np.uint8), save_dir+'/test_hr%d.jpg' % imid)

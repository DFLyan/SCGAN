...
def evaluate():
    ...
    save_dir = ("samples/fmnist/%d" % size_y).format(tl.global_flag['mode'])
    ...
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_fmnist_g.npz', network=net_g)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='evaluate_fmnist', help='evaluate_fmnist')

    args = parser.parse_args()

    tl.global_flag['mode'] = args.mode

    if tl.global_flag['mode'] == 'evaluate_fmnist':
        evaluate()
    else:

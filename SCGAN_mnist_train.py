...
def train():
    save_dir_ginit = ("samples/mnist_init_%d" % size_y).format(tl.global_flag['mode'])
    save_dir_scgan = ("samples/mnist_g_%d" % size_y).format(tl.global_flag['mode'])
    tl.files.exists_or_mkdir(save_dir_ginit)
    tl.files.exists_or_mkdir(save_dir_scgan)
    checkpoint_dir = "checkpoint/mnist_%d" % size_y
    tl.files.exists_or_mkdir(checkpoint_dir)

    train_hr_img = load_mnist(split='train')
    ...
    net_scgan = scgan_m(y_image,is_train=True, reuse=False)
    ...
    scgan_test = scgan_m(y_image, is_train=False, reuse=True)
    ...
    writer_init = tf.summary.FileWriter("logs/mnist/%d/init" % size_y, tf.get_default_graph())
    writer_final = tf.summary.FileWriter("logs/mnist/%d/final" % size_y, tf.get_default_graph())

    tl.layers.initialize_global_variables(sess)
    if tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_mnist_g.npz'.format(tl.global_flag['mode']), network=net_scgan) is False:
        tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir + '/g_mnist_init.npz'.format(tl.global_flag['mode']),network=net_scgan)
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/d_mnist.npz'.format(tl.global_flag['mode']), network=net_d)


###============================= TRAINING ===============================###
    sample_imgs = load_mnist(split='val')
    ...
    for epoch in range(0,n_epoch_init+1):
        ...
        for idx in range(0, int(train_hr_img.shape[0] // batch_size)):
            ...
            b_imgs = tl.prepro.threading_data(b_imgs, fn=norm_0)
        ...
        ## save model
        if (epoch != 0) and (epoch % 1 == 0):
            tl.files.save_npz(net_scgan.all_params, name=checkpoint_dir + '/g_mnist_init.npz'.format(tl.global_flag['mode']),sess=sess)
    ...
    for epoch in range(0,n_epoch_scgan+1):
        ...
        if (epoch != 0) and (epoch % 5 == 0):
            tl.files.save_npz(net_scgan.all_params, name=checkpoint_dir + '/g_mnist_g.npz'.format(tl.global_flag['mode']), sess=sess)
            tl.files.save_npz(net_d.all_params, name=checkpoint_dir+'/d_mnist.npz'.format(tl.global_flag['mode']), sess=sess)
